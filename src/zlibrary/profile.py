...
class ZlibProfile:
    ...
    async def get_limits(self):
        ...
        response = {
            "books": {"daily": books_daily.text, "total": books_total.text},
            "articles": {"daily": articles_daily.text, "total": articles_total.text},
        }
        return response

    async def download_history(
        self, page: int = 1, date_from: date = None, date_to: date = None
    ):
        if date_from:
    ...
    async def search_public_booklists(
        self, q: str, count: int = 10, order: OrderOptions = ""
    ):
        if order:
